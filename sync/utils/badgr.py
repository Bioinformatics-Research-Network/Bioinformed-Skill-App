    ...
    try:
        for assertion in assertionlst:
            ...
            if current_assertion.first() is None:
                ...
                print("Not in database - " + assertion.recipient_plaintextIdentity + " - " + assertion.badgeclass)
                db_session.add(assertion)
                db_session.commit()
            else:
                print("Already in database (updating) - " + fields['recipient_plaintextIdentity'] + " - " + fields['badgeclass'])
