...
def test_assessment_id_tracker(
    db:Session
    ):
    assessment = db.query(models.Assessments)\
...
def test_init_assessment_tracker(
    db: Session
    ):
    ...
    assessment = schemas.assessment_tracker_init(assessment_name=assessment_name, latest_commit=commit)

    initiate_assessment = init_assessment_tracker(db=db, assessment_tracker=assessment, user_id=1)
...
def test_update_assessment_log(
    db: Session
    ):
    assessment_name = db.query(models.Assessments)\
        .filter(models.Assessments.assessment_id == 1)\
    ...
    update_logs = update_assessment_log(db=db, asses_track_info=assessment, update_logs=test_log)
